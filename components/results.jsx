...
function Inspector({ field, results }) {
  ...
  // highlight value within snippet
      ...
      <table className="lineitems">
        ...
        <tbody>
          ...
          style={{ cursor: 'pointer', background: selectedId === it.id ? 'var(--accent-soft)' : undefined }}>
              ...
              <td className="num" style={{ color: it.unitPrice < 0 ? 'var(--bad)' : undefined }}>
                {it.unitPrice < 0 ? '-' : ''}${Math.abs(it.unitPrice).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </td>
              {!isStatement && <td className="num">${it.total.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>}
        ...
        <tfoot>
            <tr>
              ...
              {isStatement && <td className="num" style={{ color: items.reduce((s, i) => s + i.total, 0) < 0 ? 'var(--bad)' : undefined }}>
                {items.reduce((s, i) => s + i.total, 0) < 0 ? '-' : ''}${Math.abs(items.reduce((s, i) => s + i.total, 0)).toLocaleString('en-US', { minimumFractionDigits: 2 })}
              </td>}
              {!isStatement && <td className="num"></td>}
              {!isStatement && <td className="num">${items.reduce((s, i) => s + i.total, 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>}
              <td colSpan={2}></td>
            </tr>
          </tfoot>
        }
...
function Results({ results, file, onReset, toast }) {
  const [tab, setTab] = useStateRS('fields'); // fields | items | json
  const [selectedField, setSelectedField] = useStateRS(results.fields[0]);
  const [pageFilter, setPageFilter] = useStateRS(null);

  // Show ALL pages 1..N — pages with no extracted data are dimmed (not hidden)
  // so a 12-page doc still shows 12 chips.
  const pagesWithData = useMemoRS(() => {
    const all = new Set();
    results.fields.forEach((f) => all.add(f.page));
    results.lineItems.forEach((l) => all.add(l.page));
    return all;
  }, [results]);
  const pages = useMemoRS(() => {
    return Array.from({ length: results.documentMeta.pages }, (_, i) => i + 1);
  }, [results.documentMeta.pages]);
    ...
    <div>
      ...
      <div className="doc-meta-strip">
        ...
        <div className="dms-cell">
          ...
          <div className="dms-value" data-comment-anchor="6268629d08-div-222-11">{results.documentMeta.pages}</div>
        </div>
        <div className="dms-cell">
        ...
        <div className="results-card">
          ...
          <div className="fields-list">
            ...
            <div style={{ padding: '40px 20px', textAlign: 'center', color: 'var(--text-3)', fontSize: 13, gridColumn: '1/-1' }}>
                  No header fields on page {pageFilter}.
                </div> :
            visibleFields.map((f) =>
          ...
          <LineItemsTable
            ...
            onSelectItem={(it) => setSelectedField({
              ...
              value: `${it.qty || ''} ${it.unit || ''} · $${Math.abs(it.unitPrice).toLocaleString('en-US', { minimumFractionDigits: 2 })}`.trim(),
...
{JSON.stringify({ documentMeta: results.documentMeta, docType: results.docType, fields: results.fields.slice(0, 4), lineItems: results.lineItems.slice(0, 3), '__truncated_for_display': true }, null, 2)}
            </pre>
          }
        </div>
