...
const { useState: useStateAN, useRef: useRefAN, useEffect: useEffectAN } = React;
...
function buildDocContext(results) {
  // Compact context for the LLM — fields + line items summarized
  return {
...
function AnalysisPanel({ results, modelSettings, onConfigureModel, toast }) {
  const [messages, setMessages] = useStateAN([]);
  const [input, setInput] = useStateAN('');
  const [busy, setBusy] = useStateAN(false);
  const [analyseOpen, setAnalyseOpen] = useStateAN(false);
  const scrollRef = useRefAN(null);
      ...
      <div className="ap-hd">
        ...
        <div className="ap-hd-actions">
          <button className="btn btn-sm" onClick={onConfigureModel}>
            ...
            {modelConfigured ? 'Model: ' + (window.MODEL_PROVIDERS[modelSettings.provider]?.label.split(' ')[0] || 'configured') : 'Configure model'}
          </button>
...
function AnalysePopup({ results, modelSettings, onClose, toast }) {
  ...
  const [busy, setBusy] = useStateAN(true);
      ...
      <div className="modal modal-wide" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
        <div className="modal-hd">
          <div className="mh-text">
            ...
            <p>{results.documentMeta.filename} · {results.documentMeta.pages} pages · model {window.MODEL_PROVIDERS[modelSettings.provider]?.label}</p>
          </div>
        ...
        <div className="modal-body analyse-body">
          ...
          <div className="analyse-loading">
              ...
              <div>
                <div className="al-title">Analysing document with {window.MODEL_PROVIDERS[modelSettings.provider]?.label}…</div>
...
function TablesChart({ tables, totalPages }) {
  const [view, setView] = useStateAN('graphs'); // graphs | table
  ...
  return (
    <div className="tables-chart">
      ...
      <div className="tc-summary" data-comment-anchor="b900cc065d-div-435-7">
...
// ─── Mini column chart: tables count per page ───
