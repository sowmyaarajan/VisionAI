...
import { MODEL_PROVIDERS } from '../api/llm.js';

export function ModelSettingsModal({ open, settings, onChange, onClose, onTest, testState }) {
  ...
  const cfg = MODEL_PROVIDERS[draft.provider] || MODEL_PROVIDERS.custom;
  const set = (k, v) => setDraft((d) => ({ ...d, [k]: v }));

  const setProvider = (p) => {
    const newCfg = MODEL_PROVIDERS[p];
    setDraft((d) => ({
      ...d,
      provider: p,
      endpoint: '',
        ...
        <div className="modal-body">
          <div className="form-grid">
            ...
            <div className="info-note full">
              ...
              <div>
                <strong>Server-side proxy:</strong> LLM calls go through the local backend, so OpenAI / Azure / Anthropic all work without browser-CORS workarounds.
              </div>
            </div>
          </div>
        </div>
